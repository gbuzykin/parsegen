    ...
    try {
        ...
        if (std::ofstream ofile(defs_file_name); ofile) {
            ...
            for (const auto& [name, id] : grammar.getTokenList()) {
                ofile << "    tt_" << name;
                if (id > last_tt_id + 1) { ofile << " = " << id; }
                ofile << "," << std::endl;
                last_tt_id = id;
            }
            ofile << "    total_token_count," << std::endl;
            ofile << "};" << std::endl;

            ofile << std::endl << "enum {" << std::endl;
            ofile << "    predef_act_shift = 0," << std::endl;
            ofile << "    predef_act_reduce = 1," << std::endl;
            unsigned last_act_id = 0;
            for (const auto& [name, id] : grammar.getActionList()) {
                ofile << "    act_" << name;
                if (id != last_act_id + 1) { ofile << " = " << id + 1; }
                ofile << "," << std::endl;
                last_act_id = id;
            }
            ofile << "    total_action_count," << std::endl;
            ofile << "};" << std::endl;

            if (const auto& start_conditions = grammar.getStartConditions(); !start_conditions.empty()) {
                ofile << std::endl << "enum {" << std::endl;
                ofile << "    sc_" << start_conditions[0].first << " = 0," << std::endl;
                for (size_t i = 1; i < start_conditions.size(); ++i) {
                    ofile << "    sc_" << start_conditions[i].first << "," << std::endl;
                }
                ofile << "};" << std::endl;
            }

            ofile << std::endl
                  << "int parse(int tt, std::vector<int>& state_stack, unsigned& rlen, int rise_error);" << std::endl;
        } else {
