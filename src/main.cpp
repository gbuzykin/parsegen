...
template<typename Iter>
void outputArray(util::iobuf& outp, std::string_view array_name, Iter from, Iter to) {
    if (from == to) { return; }
    ...
    util::fprintln(outp, "{}[{}] = {{", array_name, std::distance(from, to));
    outputData(outp, from, to, 4);
    outp.write("};").endl();
}

void outputParserEngine(util::iobuf& outp) {
    // clang-format off
...
int main(int argc, char** argv) {
    try {
        ...
        if (!report_file_name.empty()) {
            if (util::filebuf ofile(report_file_name.c_str(), "wt"); ofile) {
        ...
        if (util::filebuf ofile(defs_file_name.c_str(), "wt"); ofile) {
            ...
            util::fprintln(ofile, "    predef_tt_error = {},", static_cast<int>(kTokenError));
            unsigned last_tt_id = kTokenError;
            for (const auto& [name, id] : grammar.getTokenList()) {
                util::fprint(ofile, "    tt_{}", name);
                if (id > last_tt_id + 1) { util::fprint(ofile, " = {}", id); }
                ofile.put(',').endl();
                last_tt_id = id;
            }
            ...
            for (const auto& [name, id] : grammar.getActionList()) {
                util::fprint(ofile, "    act_{}", name);
                if (id != last_act_id + 1) { util::fprint(ofile, " = {}", id + 1); }
                ofile.put(',').endl();
                last_act_id = id;
            }
            ...
            if (const auto& start_conditions = grammar.getStartConditions(); !start_conditions.empty()) {
                ofile.endl().write("enum {").endl();
                util::fprintln(ofile, "    sc_{} = 0,", start_conditions[0].first);
                for (size_t i = 1; i < start_conditions.size(); ++i) {
                    util::fprintln(ofile, "    sc_{},", start_conditions[i].first);
                }
                ofile.write("};").endl();
            }
