...
void outputParserEngine(std::ostream& outp) {
    ...
    static constexpr std::string_view text[] = {
        ...
        "        do {",
        "            const int* action_tbl = &action_list[action_idx[ctx.state_stack.back()]];",
        "            while (action_tbl[0] >= 0 && action_tbl[0] != std_tt_error) { action_tbl += 2; }",
...
int main(int argc, char** argv) {
    try {
        ...
        Grammar grammar;
        Parser parser(ifile, grammar);
        int res = parser.parse();
        if (res != 0) {
            std::cerr << std::endl << parser.getErrorString() << std::endl;
            return res;
        }
        ...
        if (std::ofstream ofile(defs_file_name); ofile) {
            ...
            ofile << "    std_tt_end = " << idEnd << "," << std::endl;
            ofile << "    std_tt_error = " << idError << "," << std::endl;
            int last_id = idError;
            ...
            if (const auto& action_list = grammar.getActionList(); !action_list.empty()) {
                ofile << std::endl << "enum {" << std::endl;
                int last_id = action_list.front().second;
        ...
        if (std::ofstream ofile(analyzer_file_name); ofile) {
            ...
            int prod_no, prod_count = (int)grammar.grammar_idx.size() - 1;
            for (prod_no = 0; prod_no < prod_count; prod_no++) {
                int left = grammar.grammar[grammar.grammar_idx[prod_no]];
                assert(left & maskNonterm);
                reduce_info.push_back(grammar.grammar_idx[prod_no + 1] - grammar.grammar_idx[prod_no] - 1);  // Length
                reduce_info.push_back(goto_idx[left & ~maskNonterm]);   // Goto index
                reduce_info.push_back(grammar.act_on_reduce[prod_no]);  // Action on reduce
            }
